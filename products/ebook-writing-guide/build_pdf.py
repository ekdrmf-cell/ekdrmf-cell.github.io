...
toc_line("부록5. 찾아보기(색인) ㆍ 부록6. 정보 구조화형 주제 아이디어 50가지", "toc")
...
# 11부
# ============================================================
part_page("PART 11", "실전 사례 여섯 가지 유형", "아래 사례는 이해를 돕기 위해 만든 가상의 예시이며, 실제 인물이 아닙니다.")
...
body("특정 자격시험을 준비하며 만든 오답노트를 정리해 &ldquo;시험 3주 전 압축 요약노트&rdquo;로 만든 경우입니다. 기출문제(공개 자료)를 기반으로 했기 때문에 저작권 문제가 없고, 같은 시험을 준비하는 사람들 사이에서 꾸준히 검색되는 주제라 장기적으로 판매가 이어지는 편입니다.")
body("<i>가상의 결과 예시: 시험 시행 시기(접수 시작 직전)에 맞춰 출시해 첫 달에 노출이 집중됐고, 이후로는 다음 회차 시험 접수 시기마다 다시 검색량이 오르는 계절성이 나타났습니다.</i>")
h2("사례 B. 업무 툴 활용러 — 실습형")
body("회사에서 매일 쓰는 스프레드시트 자동화 기능을 정리해 전자책으로 만든 경우입니다. 스크린샷과 함께 &ldquo;따라 하면 되는&rdquo; 형태로 구성해 실용성이 높고, 실제로 써보고 궁금한 점을 묻는 후기가 많이 달려 개정판을 자주 낼 수 있었습니다.")
body("<i>가상의 결과 예시: 후기로 달린 질문들을 모아 &ldquo;자주 묻는 질문&rdquo; 챕터를 두 달 만에 추가했고, 이 챕터가 오히려 다음 구매자들에게 &ldquo;질문에 성실히 답해주는 책&rdquo;이라는 인상을 줬습니다.</i>")
h2("사례 C. 저가 경쟁 실패 사례")
body("이미 포화 상태인 주제(범용적인 &ldquo;SNS 운영 꿀팁&rdquo; 등)를 차별점 없이 요약만 해서 최저가로 등록했다가, 비슷한 상품이 너무 많아 노출조차 되지 않은 경우입니다. 이 사례에서 얻는 교훈은 <b>가격보다 &ldquo;남과 다른 각도&rdquo;가 먼저</b>라는 것입니다.")
body("<i>가상의 결과 예시: 3-2장의 비교표 방법을 뒤늦게 적용해 경쟁 상품과 다른 각도(특정 업종 전용으로 좁히기)로 컨셉을 바꾼 뒤에야 검색 노출이 눈에 띄게 늘었습니다.</i>")
h2("사례 D. 플랫폼 다각화 사례")
body("크몽에서 먼저 반응을 확인한 뒤, 같은 콘텐츠를 부크크에서 정식 ISBN 책으로도 등록해 &ldquo;정식 출간 도서&rdquo;라는 신뢰도를 더한 경우입니다. 두 채널의 가격을 다르게 책정(부크크 쪽을 소폭 높게)해 채널 간 카니발라이제이션(자기잠식)을 줄였습니다.")
body("<i>가상의 결과 예시: 크몽 쪽 구매자는 &ldquo;바로 받아보는 PDF&rdquo;를 선호했고, 부크크 쪽 구매자는 &ldquo;정식 ISBN이 있는 책&rdquo;이라는 점을 후기에서 더 자주 언급해, 같은 콘텐츠라도 채널별로 다른 이유로 선택된다는 걸 확인했습니다.</i>")
h2("사례 E. 개정판 전략 사례")
body("첫 출시 후 들어온 질문과 후기를 모아 3개월 뒤 개정판을 내면서 &ldquo;개정 2판, 최신 정보 반영&rdquo;이라는 문구로 재홍보한 경우입니다. 기존 구매자에게는 무료 업데이트를 제공해 신뢰를 쌓고, 신규 구매자에게는 &ldquo;계속 관리되는 책&rdquo;이라는 인상을 줬습니다.")
body("<i>가상의 결과 예시: 개정판 소식을 기존 구매자에게 안내한 것만으로 몇몇이 자발적으로 후기를 다시 남겨줘, 별도 마케팅 없이도 신뢰도 지표(후기 수)가 함께 올라갔습니다.</i>")
...
# 부록6
# ============================================================
h1("부록6. 정보 구조화형 주제 아이디어 50가지")
body("무엇을 써야 할지 전혀 감이 안 잡힌다면, 아래 목록에서 자신의 경험과 겹치는 항목이 있는지 훑어보세요. 그대로 쓰기보다는, 2부의 방법으로 실제 수요를 먼저 확인한 뒤 자신의 경험을 더해 구체화하는 용도로 참고하시길 권합니다.")
idea_groups = [
    ("자격증ㆍ시험 계열", [
    ...
    ("업무 툴ㆍ생산성 계열", [
        "스프레드시트 자동화 함수 모음", "문서 작성 단축키ㆍ서식 정리", "협업 툴 활용 절차", "회의록ㆍ보고서 템플릿 활용법",
        "이메일ㆍ메신저 정리 노하우", "일정 관리 도구 세팅법", "데이터 정리ㆍ시각화 기초", "반복 업무 자동화 아이디어 모음",
        "재택근무 환경 세팅 가이드", "신입이 첫 3개월에 알아야 할 툴 정리",
    ]),
    ("온라인 판매ㆍ부업 계열", [
        "특정 플랫폼 첫 상품 등록 가이드", "상세페이지 구성 요령", "고객 응대 스크립트 모음", "정산ㆍ세금 절차 정리",
        "사입ㆍ소싱 채널 비교", "반품ㆍ교환 처리 노하우", "광고 예산 운용 기초", "리뷰 관리 전략",
        "시즌별 판매 전략 캘린더", "부업 시작 전 준비물 체크리스트(정보형으로 서술)",
    ]),
    ("마케팅ㆍSNS 계열", [
        "카드뉴스 기획 순서", "짧은 영상 스크립트 쓰는 법", "블로그 SEO 기초", "해시태그 전략",
        "팔로워 없이 시작하는 계정 운영법", "댓글ㆍDM 소통 노하우", "협찬ㆍ제휴 제안서 쓰는 법",
        "브랜드 톤앤매너 잡는 법", "콘텐츠 소재 찾는 습관", "광고 vs 오가닉 노출 차이 이해하기",
    ]),
    ("생활ㆍ취미 정보 계열", [
        "특정 취미 입문 장비 고르는 법", "동호회ㆍ커뮤니티 참여 가이드", "관련 용어 사전",
        "초보자가 자주 하는 실수 정리", "예산별 시작 가이드", "관리ㆍ유지보수 노하우",
        "관련 자격ㆍ대회 정보 정리", "안전 수칙 정리", "장비 비교표", "단계별 숙련 로드맵",
    ]),
    ("육아ㆍ가정 살림 계열", [
        "월령별 육아용품 체크 정보", "이유식 시기별 진행 정리", "어린이집ㆍ유치원 선택 정보 정리",
        "정부 육아 지원제도 정보 정리", "가계부 항목 분류법", "정리정돈 공간별 노하우",
        "명절ㆍ행사 준비 체크 정보", "가전제품 관리 요령 정리", "육아휴직 절차 정보 정리", "초등 입학 준비 정보 정리",
    ]),
    ("외국어ㆍ자기계발 계열", [
        "특정 어학시험 파트별 공략법", "일상 회화 자주 쓰는 표현 정리", "발음 교정 포인트 정리",
        "어학 스터디 운영 노하우", "자격 취득 후 활용 분야 정리", "독학 루틴 설계법",
        "필사ㆍ낭독 학습법 정리", "어휘 암기 효율화 방법", "원서ㆍ뉴스 활용 학습법", "면접ㆍ자기소개 표현 정리",
    ]),
]
for group_name, items in idea_groups:
    ...
    story.append(Spacer(1, 8))
body("이 50가지는 전부 <b>공개된 정보ㆍ직접 경험을 정리하는 정보 구조화형(5장 참고)</b>으로 분류될 수 있는 방향입니다. 다만 세무ㆍ법률ㆍ의료처럼 자격이 필요한 영역과 맞닿는 세부 주제는 5장의 체크리스트를 다시 한번 확인하고 시작하세요.")
