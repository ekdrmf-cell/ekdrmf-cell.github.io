...
// 2026-08-30 수정 — 사용자 지시: build_report.py의 삽입 형식을 "용어(뜻풀이)" →
// "뜻풀이(용어)"로 뒤집으면서, 완결된 문장체("~합니다") 대신 뒤에 오는 명사(기운)를
// 수식하는 관형구로 다시 씀 — 개념ㆍ정보는 그대로, 문장 구조만 바꿈(자동 diff 검증
// 완료, scratchpad/gloss_transform_validate.py 참고).
const SHENSHA_MEANING = {
  도화살: "이성에게 매력적으로 비치고, 예전에는 다소 조심스럽게 여겨졌지만 요즘은 인기와 매력, 사교성으로 좋게 해석되곤 하는 기운",
  역마살: "이동과 변화가 많으며, 이사나 출장ㆍ해외 생활ㆍ이직처럼 한곳에 머무르지 않고 움직이는 삶과 관련이 깊다고 전통적으로 여겨지는 기운",
  화개살: "학문이나 예술, 종교처럼 정신적인 세계에 깊이 빠져들게 하고, 오히려 혼자만의 시간에 집중력과 창의력이 살아나는 성향으로 전통적으로 여겨지는 기운",
  홍염살: "도화살처럼 이성에게 인기 있는 매력을 뜻하되 좀 더 화려하고 표현이 강한 쪽에 가까우며, 옛 문헌에서는 여성의 사주에서 더 자주 언급됐지만 요즘은 성별에 관계없이 함께 다뤄지는 기운",
};
